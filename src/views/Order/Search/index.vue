...
const CONST_COLUMNS = [
  ...
  { key: "id", title: "订单编号", width: 125 },
  {
    ...
    render: function(h, params) {
      return h(
        "div",
        Util.utcToString(params.row.createTime, "yyyy-MM-dd hh:mm:ss")
      );
    }
  },
  {
  ...
  { key: "customerPhone", title: "新人电话", width: 150 },
  {
    ...
    render: function(h, params) {
      return h(
        "div",
        Util.utcToString(params.row.workTime, "yyyy-MM-dd hh:mm")
      );
    }
  },
  ...
  { key: "packageName", title: "套餐名称", width: 150 },
  {
    ...
    render: function(h, params) {
      return h(
        "span",
        (parseInt(params.row.amountTotal) / 100).toFixed(2) + "元"
      );
    }
  },
  {
    ...
    render: function(h, params) {
      return h(
        "span",
        (parseInt(params.row.amountDiscount) / 100).toFixed(2) + "元"
      );
    }
  },
...
export default {
  ...
  methods: {
    ...
    search(filter, index, size) {
      ...
      this.$root.$axios.get(Config.api.order.query, option, response => {
        self.loading = false;
