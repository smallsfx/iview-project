...
router.beforeEach((to, from, next) => {
  ...
  if (localStorage.locking === 1 && to.name !== 'locking') { // 判断当前是否是锁定状态
    ...
    if (!localStorage.accessToken && to.name !== 'login') { // 判断是否已经登录且前往的页面不是登录页
      next({ name: 'login' });
    } else if (localStorage.accessToken && to.name === 'login') { // 判断是否已经登录且前往的是登录页
...
router.afterEach((to) => {
  router.app.$store.dispatch("openNewPage", to.name, to.params, to.query);
